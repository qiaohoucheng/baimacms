...
class LinkController extends Controller
{
    ...
    {
        if($request->ajax()){
            return  $this->dataFormatMany(new Link(),$request,[],'post');
        }
    ...
    public function store(Request $request)
    {
        ...
        if ($validator->fails()){
            ...
            if(isset($r->id) && $r->id >0){
                Link::where('id',$r->id)->update(['sort'=>$r->id]);
    ...
    {
        $info = Link::with('post')->find($id)->toArray();
    ...
    public function update(Request $request,$id)
    {
        ...
        if ($validator->fails()){
            ...
            $r = Link::where('id',$id)->update($post);
            if($r >0){
